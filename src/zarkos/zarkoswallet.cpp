...
CzARKOSWallet::CzARKOSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zarkos seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZARKOSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZARKOSSeed_deprecated()) {
                    LogPrintf("%s: Updated zARKOS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zarkos seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zarkos wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzARKOSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZARKOSCount(nCountLastUsed);
    else if (!walletdb.ReadZARKOSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzARKOSWallet::Lock()
{
    seedMaster = 0;
}

void CzARKOSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzARKOSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZARKOS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzARKOSWallet::LoadMintPoolFromDB()
{
...
void CzARKOSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzARKOSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzARKOSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zarkosTracker->HasPubcoinHash(pMint.first)) {
...
bool CzARKOSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZARKOS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zarkosTracker which also adds to database
    pwalletMain->zarkosTracker->Add(dMint, true);
...
void CzARKOSWallet::SeedToZARKOS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzARKOSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzARKOSWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZARKOSCount(nCountLastUsed);
...
void CzARKOSWallet::GenerateDeterministicZARKOS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzARKOSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzARKOSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzARKOSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
