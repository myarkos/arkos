...
CzARKOSTracker::CzARKOSTracker(std::string strWalletFile)
{
...
CzARKOSTracker::~CzARKOSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzARKOSTracker::Init()
{
...
bool CzARKOSTracker::Archive(CMintMeta& meta)
{
...
bool CzARKOSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzARKOSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzARKOSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzARKOSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzARKOSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzARKOSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzARKOSTracker::GetSerialHashes()
{
...
CAmount CzARKOSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzARKOSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzARKOSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzARKOSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzARKOSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzARKOSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzARKOSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzARKOSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzARKOSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzARKOSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzARKOSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzARKOSWallet* zARKOSWallet)
{
    bool iszARKOSWalletInitialized = (NULL != zARKOSWallet);
    CMintMeta meta;
    ...
    if (! iszARKOSWalletInitialized)
        zARKOSWallet = new CzARKOSWallet(strWalletFile);
    meta.isSeedCorrect = zARKOSWallet->CheckSeed(dMint);
    if (! iszARKOSWalletInitialized)
        delete zARKOSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzARKOSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzARKOSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzARKOSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzARKOSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzARKOSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzARKOSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzARKOSWallet* zARKOSWallet = new CzARKOSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zARKOSWallet);
        }
        delete zARKOSWallet;
        LogPrint("zero", "%s: added %d dzarkos from DB\n", __func__, listDeterministicDB.size());
    }
