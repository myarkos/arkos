...
#include "ui_zarkoscontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zarkosTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zarkosTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zARKOS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zARKOS seed used to mint this zARKOS is not the same as currently hold in the wallet";
            else
