...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZARKOSModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zarkosTracker->HasMintTx(hash))) {
            //zARKOS stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZARKOS;
...
bool IsZARKOSType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZARKOS:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZARKOS || type == TransactionRecord::MNReward) {
