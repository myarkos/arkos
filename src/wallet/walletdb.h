...
public:
    ...
    bool WriteZARKOSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZARKOSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZARKOSSeed_deprecated(uint256& seed);
    bool EraseZARKOSSeed();
    bool EraseZARKOSSeed_deprecated();

    bool WriteZARKOSCount(const uint32_t& nCount);
    bool ReadZARKOSCount(uint32_t& nCount);
