...
/**
 * ARKOSCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZarkosSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzarkosseed(const UniValue& params, bool fHelp);
extern UniValue getzarkosseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzarkos(const UniValue& params, bool fHelp);
extern UniValue dzarkosstate(const UniValue& params, bool fHelp);
