...
// The zARKOS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zARKOS is a hash of the serial
...
{
    //Create an output returning the zARKOS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZARKOSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zARKOS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zARKOS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZARKOSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zARKOS output", __func__);
...
{
    CzARKOSTracker* zarkosTracker = pwallet->zarkosTracker.get();
    CMintMeta meta;
    if (!zarkosTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zarkosTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ARKOS stake is the outpoint
