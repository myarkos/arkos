...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzARKOSWallet *czARKOSWallet = new CzARKOSWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czARKOSWallet->GenerateDeterministicZARKOS(denom, coin, dMint, true);
        czARKOSWallet->UpdateCount();
